...
// If start is called a second time while first start is still running, duplicate processes will be spawned.
// Added lock to prevent using start while already starting or to restart
// To enable using start command for restarting pm2, we would need to set startLocked back to false after start command is done
var startLocked = false
God.checkStatus = function checkStatus(env, cb) {
  var wasStartLocked = startLocked
  startLocked = true
  cb(null, wasStartLocked)
}
  ...
  async.timesLimit(env.instances, 1, function (n, next) {
    ...
    God.injectVariables(env, function inject (err, _env) {
      ...
      return God.executeApp(Utility.clone(_env), function (err, clu) {
        ...
        return setTimeout(function() { next(err, Utility.clone(clu)); }, env.finale_pace_restart);
      });
    });
  }, cb);
};
