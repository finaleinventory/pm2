...
////////////////////////////
  ...
  start (cmd, opts, cb) {
    ...
    that.Client.executeRemote('checkStatus', {}, function(err, startLocked) {
      if (err) {
        Common.printError(conf.PREFIX_MSG_ERR + 'Error while checking status before starting', err.stack || err);
        return cb ? cb({success:false, msg:err}) : that.exitCli(conf.ERROR_EXIT)
      }

      if (startLocked) {
        Common.printOut(conf.PREFIX_MSG_WARNING + 'Already started, use restart command instead.')
        return cb ? cb({success:false, msg:'PM2 already started, use restart command instead.'}) : that.exitCli(conf.ERROR_EXIT)
      }
  ...
  _startJson (file, opts, action, pipe, cb) {
    ...
    appConf.forEach(function(app) {
      ...
      // Namespace
      if (!opts.namespace && !app.namespace)
  ...
  speedList (code, list) {
    ...
    return that.Client.executeRemote('getSystemData', {}, (err, sys_infos) => {
      if (list) {
        return doList(null, list, sys_infos)
      }
